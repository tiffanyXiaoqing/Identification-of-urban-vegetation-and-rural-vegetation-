...
void ClassifyVegetation(int* buffer, const int* urbanDensity, int width, int height, int height_start, int height_end, std::set<int> vegClassSet, int urbanVegThres, int maxFloodCount, int urbanVegClassNum, int ruralVegClassNum) {
	...
	for (int y = height_start; y < height_end; y++) {
		...
		for (int x = 0; x < width; x++) {
			...
			if (vegClassSet.find(buffer[cell]) != vegClassSet.end()) {
				...
				memcpy(bufferCopy, buffer, sizeof(int)* width * height);
				...
				if (urbanDensity[cell] >= urbanVegThres) {
					...
					if (floodfiller.FloodFill(bufferCopy, x, y, height_start, height_end, vegClassSet, urbanVegClassNum, 0) > maxFloodCount) {
						floodfiller.FloodFill(buffer, x, y, height_start, height_end, vegClassSet, ruralVegClassNum, -1);
					}
					// Or floodfill this area as urban vegetation
					else {
						memcpy(buffer, bufferCopy, sizeof(int)* width * height);
				...
				// If the pixel is located at low value area in UDM
					...
					if (floodfiller.FloodFill(bufferCopy, x, y, height_start, height_end, vegClassSet, ruralVegClassNum, 0) > maxFloodCount) {
						floodfiller.FloodFill(buffer, x, y, height_start, height_end, vegClassSet, ruralVegClassNum, -1);
						count++;
						printf("Filled %d areas.\n", count);
					}
